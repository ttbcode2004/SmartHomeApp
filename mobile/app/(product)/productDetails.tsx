import { View, Text, Image, ScrollView, TouchableOpacity, StatusBar, ActivityIndicator, Dimensions, Modal} from "react-native";
...
export default function ProductDetailsScreen() {
  ...
  const { fetchProductById, fetchRelated, selectedProduct, relatedProducts } =
    useProducts();
  ...
  useEffect(() => {
    if (!id) return;
    Promise.all([fetchProductById(id), fetchRelated(id)]).finally(() =>
      setIsLoading(false),
    );
  }, [id]);
  ...
  const handleUserSelect = async () => {
    const chat = await getOrCreateChat(product.user._id);
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: colors.bg }}>
      ...
      >
        ...
        <View className="p-4 gap-3.5">
          ...
          <TouchableOpacity
            ...
            style={{
              backgroundColor: isWishlisted(product._id)
                ? colors.danger + "15"
                : colors.surface,
              borderWidth: 1,
          ...
          >
            {wishlistLoading ? (
              ...
              <Feather
                ...
                size={15}
                color={
                  isWishlisted(product._id) ? colors.danger : colors.textMuted
                }
              />
            )}
            <Text
          ...
          >
            {product.user?.profilePicture ? (
              <Image
                source={{ uri: product.user.profilePicture }}
      ...
      <Modal visible={showViewer} animationType="fade">
        <View className="flex-1 bg-black">
          ...
          <TouchableOpacity
            ...
            style={{
              top: insets.top + 10,
              backgroundColor: colors.surface + "40", // blur nhẹ theo theme
            }}
          >
